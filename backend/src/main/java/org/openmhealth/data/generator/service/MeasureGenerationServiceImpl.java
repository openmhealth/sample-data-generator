...
public class MeasureGenerationServiceImpl implements MeasureGenerationService {
    ...
    public Iterable<MeasureGroup> generateMeasureGroups(MeasureGenerationRequest request) {
        ...
        do {
            ...
            double trendProgressFraction =
                    (double) Duration.between(request.getStartDateTime(), effectiveDateTime).getSeconds() /
                            totalDurationInS;

            for (Map.Entry<String, RealValueRandomVariableTrend> trendEntry : request
                    .getMeasureValueTrends().entrySet()) {
